...
public class TurtleSoupUI : MonoBehaviour
{
    ...
    {
        gameManager = FindObjectOfType<TurtleSoupGameManager>();
        scrollRect = contentContainer.parent.parent.GetComponent<ScrollRect>();
    }
    ...
    {
        GameObject msgObj = Instantiate(playerMessagePrefab, contentContainer);
        SetupMessageBubble(msgObj, message, "玩家", playerAvatar, true);
        StartCoroutine(ScrollToBottom());
    }
    ...
    {
        GameObject msgObj = Instantiate(aiMessagePrefab, contentContainer);
        SetupMessageBubble(msgObj, message, "汤师傅", aiAvatar, false);
        StartCoroutine(ScrollToBottom());
    }

    private void SetupMessageBubble(GameObject messageObj, string message, string senderName, Sprite avatar, bool isPlayer)
    {
        // 设置头像
        ...
        // 设置发送者名称
        TMP_Text nameText = messageObj.transform.Find("MessageContainer/NameText")?.GetComponent<TMP_Text>();
        if (nameText != null)
        {
            nameText.text = senderName;
            nameText.gameObject.SetActive(!string.IsNullOrEmpty(senderName));
        }

        // 设置消息内容
        TMP_Text messageText = messageObj.transform.Find("MessageContainer/Bubble/MessageText")?.GetComponent<TMP_Text>();
        if (messageText != null)
        {
            messageText.text = message;

            // 自适应气泡大小
            RectTransform bubbleRect = messageText.transform.parent.GetComponent<RectTransform>();
            if (bubbleRect != null)
            {
                // 强制立即计算文本尺寸
                ...
                // 计算合适的宽度
                float preferredWidth = Mathf.Min(messageText.preferredWidth + 40, maxBubbleWidth);
                preferredWidth = Mathf.Max(preferredWidth, minBubbleWidth);
        ...
        // 设置布局
        HorizontalLayoutGroup layoutGroup = messageObj.GetComponent<HorizontalLayoutGroup>();
        if (layoutGroup != null)
        {
            layoutGroup.childAlignment = isPlayer ? TextAnchor.UpperRight : TextAnchor.UpperLeft;
        }
    }
    ...
    {
        yield return new WaitForEndOfFrame();
        yield return new WaitForEndOfFrame(); // 确保UI更新完成
